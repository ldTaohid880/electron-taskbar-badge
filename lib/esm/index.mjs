...
import fs from "fs";
import BadgeGenerator from "./badge_generator.mjs";
...
export default class Badge {
	constructor(win, opts = { listenFromRenderer: true }) {
		if (process.platform !== "win32")
			console.warn("Only win32 environments are supported!");
		this.win = win;
		this.opts = opts;
		...
		systemPreferences.on("accent-color-changed", () => {
			this.generator = new BadgeGenerator(
				win,
				opts,
				getLightAccentColor(powershell)
			);
		...
		if (typeof opts?.updateBadgeEvent !== "string") {
			throw new TypeError(
				`Invalid IPC event handler name specified.\nExpected: string\nGot: ${typeof opts?.updateBadgeEvent}`
			);
		}
	...
	try {
		accentColorData = JSON.parse(
			require("child_process")
				.execSync(
					`"${pwsh}" -NoProfile -Command "Get-ItemProperty -Path 'HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Accent' | Select-Object AccentPalette | ConvertTo-Json"`
				)
				.toString()
		).AccentPalette;
	} catch (error) {
